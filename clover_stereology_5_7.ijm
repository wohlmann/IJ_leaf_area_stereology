macro "Clover leaf stereology" {
		...
		for (i=0; i<list.length; i++) {
			...
			actimage = getTitle();
			title1= getTitle;
			title2 = File.nameWithoutExte			lineWidth=15;ion;
			print("analysing: "+title1+"");
			getPixelSize(unit, pw, ph, pd);
//			setTool("rectangle");
//			waitForUser("please draw a rectangle for cropping");
			Squareok=false;
			while(Squareok==false){
				if(Squareok==false){
					setTool("rectangle");
					waitForUser("please draw a rectangle for cropping");
					Dialog.create("happy with Square?");
...
//			waitForUser("please draw the calibration line (20MM)");
			...
			while(Lineok==false){
				if(Lineok==false){		
					setTool("line");
					waitForUser("please draw the calibration line (20MM)");
					Dialog.create("happy with Line?");
					items = newArray("Yes", "No");
					Dialog.addRadioButtonGroup("", items, 2, 2, "Yes");
					Dialog.show();
					Line = Dialog.getRadioButton();
						if (Line=="Yes"){
							lineWidth=5;
							Lineok=true;
						}
